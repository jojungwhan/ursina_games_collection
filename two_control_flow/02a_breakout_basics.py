...
#5 move the ball so it bounces off the walls
def update():
    ball.x += ball.dx    # ball x position increases by dx every frame
    ball.y += ball.dy    # ball y position increases by dy every frame
         # left arrow moves paddle left and right arrow moves it right
    paddle.x += (held_keys['right arrow'] - held_keys['left arrow']) * time.dt *5

    hit_info = ball.intersects()
    if hit_info.hit:     						# if ball collides with another entity      
        if hit_info.entity == left_wall or hit_info.entity == right_wall:
            ball.dx = -ball.dx			# change direction of ball on x axis
        if hit_info.entity == floor or hit_info.entity == ceiling:
            ball.dy = -ball.dy			# change direction of ball on y axis
        if hit_info.entity in bricks:
							 # destroy brick and remove from view
            destroy(hit_info.entity)
            ball.dy = -ball.dy			# change the direction of ball on y axis
        if hit_info.entity == paddle:
            ball.dy = -ball.dy			# change direction of ball on y axis
